      ...
      <div class="text-right space-y-1 mt-4 md:mt-0">
        ...
        <div class="mt-4">
          <UFormItem label="Currency:" labelPlacement="top">
            <USelect v-model="invoice.currencyCode" :options="currencyOptions" />
          </UFormItem>
        </div>
      </div>
    </div>
      ...
      <UGrid :cols="3" gap="4">
        <UGridItem>
          <UFormItem label="Terms">
            <USelect v-model="invoice.terms" :options="termsOptions" />
          </UFormItem>
        </UGridItem>
        <UGridItem>
        ...
        <table class="w-full border-collapse border-gray-300 text-sm">
          ...
          <tbody>
            <tr v-for="(row, index) in invoice.items" :key="index">
              ...
              <td class="border p-2">
                <USelect v-model="row.tax" :options="taxOptions" @update:modelValue="handleTaxChange(row)" />
              </td>
      ...
      <UGridItem :span="3">
        <UCard>
          ...
          <template #default>
            ...
            <UButton type="default" @click="addPaymentGateway">Add Payment Gateway</UButton>
          </template>
        </UCard>
      </UGridItem>
...
const currencySymbol = computed(() => {
  const option = currencyOptions.find((opt) => opt.code === invoice.currencyCode);
