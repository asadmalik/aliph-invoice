    ...
    <UContainer as="div">
      <div class="w-full flex-row items-start align-middle">
        <UFormField label="Customer Name" labelPlacement="bottom">
          <USelect :items="customers" value-key="email" label-key="name" placeholder="Select status" v-model="selectedCustomer"></USelect>
        </UFormField>
        <UBadge>{{ selectedCustomer?.currency }}</UBadge>
      </div>
