<template>
  <div>
    ...
    <UCard>
      ...
      <template #default>
        <UTable :columns="itemColumns" :data="invoice.items" class="mb-4">
  <template #cell(item)="props">
    <UInput v-model="props.row.item" placeholder="Type or select an item" />
  </template>
  <template #cell(quantity)="props">
    <UInput type="number" v-model.number="props.row.quantity" min="0" />
  </template>
  <template #cell(rate)="props">
    <UInput type="number" v-model.number="props.row.rate" min="0" />
  </template>
  <template #cell(tax)="props">
    <USelect
      v-model="props.row.tax"
      :items="taxOptions"
      @update:modelValue="handleTaxChange(props.row)"
    />
  </template>
  <template #cell(amount)="props">
    {{ formatCurrency(rowAmount(props.row)) }}
  </template>
  <template #cell(actions)="props">
    <UButton type="error" size="small" @click="removeRow(props.index)">
      X
    </UButton>
  </template>
</UTable>
...
/*   const customers = ref([
  ...
  const itemColumns = [
  { Id:0, title: 'Item', key: 'item' },
  { Id:1, title: 'Quantity', key: 'quantity' },
  { Id:2, title: 'Rate', key: 'rate' },
  { Id:3, title: 'Tax', key: 'tax' },
  { Id:4, title: 'Amount', key: 'amount' },
  { Id:x, title: 'Actions', key: 'actions' }
];
